/// 2010 10.21
// CPSO with iteration in each time setep.
...
int init_particles(PSO *pso)
{
  ...
  for(i=0;i<NofParticles;i++){
    ...
    pso->ptcl[i].st.Q=10; //full charged
  }
...
///////////////////////////////////////////////////
// RE-initialization of particles
///////////////////////////////////////////////////
int re_init_particles(PSO *pso)
{
  static int i;
  static double f;
  static CvRNG rng=cvRNG(-1);
  ...
  for(i=0;i<NEUTRAL;i++){
    pso->ptcl[i].st.x.x=250*n_rand(0,1)+250;
    pso->ptcl[i].st.x.y=250*n_rand(0,1)+250;
    pso->ptcl[i].st.v.x=n_rand(0,1);
    pso->ptcl[i].st.v.y=n_rand(0,1);
    pso->ptcl[i].st.pbest.x=pso->ptcl[i].st.x.x;
    pso->ptcl[i].st.pbest.y=pso->ptcl[i].st.x.y;
...
int time_evolution(PSO *pso, int W, int C1, int C2)
{
  ...
  static CvRNG rng=cvRNG(-1);
  static CvPoint2D32f r;
  static double r_norm,co;
  ...
  for(i=0;i<NofParticles;i++){
    ...
    pso->ptcl[i].st.v_norm=sqrt(pow(pso->ptcl[i].st.v.x,2)+
				pow(pso->ptcl[i].st.v.y,2));

    if(pso->ptcl[i].st.v_norm>v_max){
      pso->ptcl[i].st.v.x*=v_max/(double)pso->ptcl[i].st.v_norm;
      pso->ptcl[i].st.v.y*=v_max/(double)pso->ptcl[i].st.v_norm;
    } 
    ...
    // charged particles
    if(i>NEUTRAL){
      pso->ptcl[i].st.a.x=0;    pso->ptcl[i].st.a.y=0;
      for(j=0;j<NofParticles;j++){
	if(i!=j){
	  r.x=pso->ptcl[i].st.x.x-pso->ptcl[j].st.x.x;
	  r.y=pso->ptcl[i].st.x.y-pso->ptcl[j].st.x.y;
	  r_norm=pow(sqrt(pow(r.x,2)+pow(r.y,2)),3);
	  co=pso->ptcl[i].st.Q*pso->ptcl[j].st.Q/(double)r_norm;
	  pso->ptcl[i].st.a.x+=co*r.x;
	  pso->ptcl[i].st.a.y+=co*r.y;
	}
	pso->ptcl[i].st.x.x+=pso->ptcl[i].st.a.x;
	pso->ptcl[i].st.x.y+=pso->ptcl[i].st.a.y;
      }
    }
...
int main()
{
  ...
  for(cnt=0;;cnt++){    
    ...
    if(pso.f_gbest!=(ee=evaluate(pso.gbest,cnt))){
      pso.f_gbest=ee;
      re_init_particles(&pso);
    } 
    ...
    cvSaveImage(comand,img);
#endif      
    if(cnt==471) cvSaveImage("cpso.png",img);
    //ここの値を大きくすると実行が遅くなる。
    code = cvWaitKey(5);
