/// 2010 6/4
// e-PSO for moving target function. 
// The trajectly of gbest is drawn.
...
double evaluate(CvPoint2D32f p, int cnt)
{
  ...
  f=1-(exp(-0.5*(pow((p.x-bx1-125*sin(0.01*cnt)),2)/pow((s1_x),2)+pow((p.y-by1-125*cos(0.01*cnt)),2)/pow((s1_y),2))));
...
int time_evolution(PSO *pso, int w, int c1, int c2)
{
  ...
  for(i=0;i<NofParticles;i++){
    pso->ptcl[i].st.v.x=
      ww*pso->ptcl[i].st.v.x+
      cc1*cvRandReal(&rng)*(pso->ptcl[i].st.pbest.x-pso->ptcl[i].st.x.x)+
      cc2*cvRandReal(&rng)*(pso->gbest.x-pso->ptcl[i].st.x.x);

    pso->ptcl[i].st.v.y=
      ww*pso->ptcl[i].st.v.y+
      cc1*cvRandReal(&rng)*(pso->ptcl[i].st.pbest.y-pso->ptcl[i].st.x.y)+
      cc2*cvRandReal(&rng)*(pso->gbest.y-pso->ptcl[i].st.x.y);
    
    pso->ptcl[i].st.x.x+=pso->ptcl[i].st.v.x+(cvRandReal(&rng)-0.5)*R3;
    pso->ptcl[i].st.x.y+=pso->ptcl[i].st.v.y+(cvRandReal(&rng)-0.5)*R3;
...
int main()
{
  ...
  for(cnt=0;;cnt++){    
    ...
    pso.f_gbest+=eta;
    for(i=0;i<NofParticles;i++)pso.ptcl[i].st.f_pbest+=eta;
    ...
    //DrawTrajectory(tra,kal_gbest,predict_pt,blue);
    ...
    else if(100<=cnt && cnt<1000) sprintf(text,  "%d",cnt);
    sprintf(comand,"./img0922_pso5/p%s.png",text);  
    cvSaveImage(comand,img);
#endif      
    if(cnt==471) cvSaveImage("ePSO.png",img); 
...
//convert -delay 2 -loop 0 *.gif abcmod.gif
