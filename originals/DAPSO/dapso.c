...
int time_evolution(PSO *pso, int W, int C1, int C2)
{
  ...
  //phi=c1+c2;
  //if(phi<4){ phi=4;
  //  printf("warning! C1+C2 is too small.\n");}
  //w=2/(double)(2-phi-sqrt(pow(phi,2)-4*phi));
  w=W*1e-3;
...
int main()
{
  ...
  //ここからがsimulationのコア
  for(cnt=0;;cnt++){    
    cvSet(img,white,0);      //描画の初期化。背景を白色に     
    cvRectangle(img,pt1,pt2,black,1,4,0);     //枠を付ける(趣味の問題)。
    DrawAxis(img);          //軸を描画
    DrawTrue(img,cnt);      //ターゲットのマークを描画
    ...
    for(i=0;i<NofParticles;i++){
      ...
      if(f<pso.ptcl[i].st.f_pbest*eta){
	pso.ptcl[i].st.pbest=pso.ptcl[i].st.x; 
	pso.ptcl[i].st.f_pbest=f;
      }
      else pso.ptcl[i].st.f_pbest*=eta;
    }
    ...
    if(cnt==471) cvSaveImage("dapso.png",img);
